...
export const NetworkTelemetryTester: React.FC<{ compact?: boolean }> = ({ compact = false }) => {
  const [activeTab, setActiveTab] = useState<'scapy' | 'pcap' | 'simulator'>('scapy');
  ...
  const handleAnalyzeExportedPcapWithEngine5 = async () => {
    if (!exportedPcap) return;
    setActiveTab('pcap');
    await loadSamplePcap(exportedPcap.filename);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl bg-[#0F151D] border border-[#25313E] overflow-hidden shadow-lg">
        <div className="flex border-b border-[#25313E] bg-[#0A0E15] px-4 sm:px-6 gap-2 pt-2">
          ...
          >
            <span>🎛️ What-If Testing (Threat Lab)</span>
          </button>
        </div>
        ...
        {/* ========================================================================= */}
        {/* TAB 2: ANALYZE PCAP FILE */}
        {/* ========================================================================= */}
        {activeTab === 'pcap' && (
          <div className="p-4 sm:p-6 space-y-6">
            <div className="p-6 rounded-xl bg-[#080C12] border border-dashed border-[#25313E] hover:border-[#5B8DEF] transition-colors text-center space-y-3">
              <div className="w-12 h-12 rounded-full bg-[#5B8DEF]/10 border border-[#5B8DEF]/30 flex items-center justify-center mx-auto text-[#5B8DEF]">
                <Upload size={22} />
              </div>
              <div>
                <h3 className="text-sm font-bold text-[#E8EDF3]">Upload or Select Wireshark PCAP (.pcap / .pcapng)</h3>
                <p className="text-xs text-[#8996A6] mt-1">
                  Engine 5 parses packet headers, aggregates flows, and evaluates each session against the trained Random Forest model.
                </p>
              </div>

              <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
                <label className="px-4 py-2 rounded-md bg-[#5B8DEF] hover:bg-[#4a7de0] text-white text-xs font-semibold cursor-pointer transition-colors flex items-center gap-2">
                  <FileUp size={14} />
                  <span>Choose External PCAP</span>
                  <input
                    type="file"
                ...
                {exportedPcap && (
                  <button
                    onClick={handleAnalyzeExportedPcapWithEngine5}
                    disabled={loading}
                    className="px-4 py-2 rounded-md bg-[#35C98A] hover:bg-[#2EB37A] text-[#0A0E15] text-xs font-bold transition-colors flex items-center gap-1.5 cursor-pointer"
                  >
                    <Play size={13} fill="currentColor" />
                    <span>Analyze Simulation PCAP ({exportedPcap.filename})</span>
                  </button>
                )}
              </div>

              {/* Sample PCAP quick buttons */}
              <div className="pt-3 border-t border-[#25313E]/60 flex flex-wrap items-center justify-center gap-2 text-xs">
                <span className="text-[#8996A6] text-[11px] mr-1">Or test verified sample captures:</span>
                <button
                  onClick={() => loadSamplePcap('sample_attack_traffic.pcap')}
                  disabled={loading}
                  className="px-2.5 py-1 rounded bg-[#EF6262]/10 hover:bg-[#EF6262]/20 border border-[#EF6262]/30 text-[#EF6262] text-[11px] font-medium transition-colors cursor-pointer"
                >
                  🔴 Sample Attack PCAP (SYN Flood)
                </button>
                <button
                  onClick={() => loadSamplePcap('sample_normal_traffic.pcap')}
                  disabled={loading}
                  className="px-2.5 py-1 rounded bg-[#35C98A]/10 hover:bg-[#35C98A]/20 border border-[#35C98A]/30 text-[#35C98A] text-[11px] font-medium transition-colors cursor-pointer"
                >
                  🟢 Sample Normal PCAP (HTTP Web)
                </button>
              </div>
            </div>
            ...
            {pcapResult && (
              <div className="space-y-4">
                <div className="p-4 rounded-xl bg-[#121A24] border border-[#25313E] flex flex-wrap items-center justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-mono font-bold text-[#5B8DEF] px-2 py-0.5 rounded bg-[#5B8DEF]/10 border border-[#5B8DEF]/30">
                        WIRESHARK PCAP
                      </span>
                      <span className="text-sm font-bold text-[#E8EDF3]">{pcapResult.filename}</span>
                    </div>
                    <span className="text-xs text-[#8996A6] block mt-1">
                      Packets: <strong>{pcapResult.total_packets}</strong> | Flows Dissected: <strong>{pcapResult.distinct_flows_count}</strong>
                    </span>
                  </div>

                  <div className="flex items-center gap-4 text-right">
                    <div>
                      <span className="text-[10px] uppercase text-[#8996A6] block font-medium">Max Threat Score</span>
                      <span className={`text-xl font-bold font-mono ${pcapResult.max_threat_score > 60 ? 'text-[#EF6262]' : 'text-[#35C98A]'}`}>
                        {pcapResult.max_threat_score} / 100
                      </span>
                    </div>

                    <div>
                      <span className="text-[10px] uppercase text-[#8996A6] block font-medium">Anomalous Flows</span>
                      <span className={`text-sm font-bold px-2.5 py-1 rounded border inline-block ${
                        pcapResult.anomalous_flows_count > 0
                          ? 'bg-[#EF6262]/15 text-[#EF6262] border-[#EF6262]/30'
                          : 'bg-[#35C98A]/15 text-[#35C98A] border-[#35C98A]/30'
                      }`}>
                        {pcapResult.anomalous_flows_count} Detected
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-xl bg-[#080C12] border border-[#25313E] overflow-hidden">
                  <div className="p-3 bg-[#121A24] border-b border-[#25313E] flex items-center justify-between">
                    <span className="text-xs font-bold text-[#E8EDF3] uppercase tracking-wider">
                      TOP SUSPICIOUS FLOWS (ENGINE 5 CLASSIFICATION)
                    </span>
                    <span className="text-[11px] text-[#8996A6]">Click any flow to inspect</span>
                  </div>

                  <div className="overflow-x-auto">
                    <table className="w-full text-xs text-left">
                      <thead className="bg-[#0D131C] text-[#8996A6] text-[10px] uppercase border-b border-[#25313E]">
                        <tr>
                      ...
                      <tbody className="divide-y divide-[#25313E]/40 text-[11px]">
                        {pcapResult.analyzed_flows_sample.map((flow, idx) => (
                          <tr
                            key={idx}
                            onClick={() => setPcapSelectedFlow(flow)}
                            className="hover:bg-[#121A24]/60 transition-colors cursor-pointer"
                          >
                            <td className="p-3 font-mono text-[#E8EDF3] font-medium">{flow.source_ip || flow.source}</td>
                            <td className="p-3 font-mono text-[#5B8DEF]">{flow.destination_ip || flow.destination}</td>
                            <td className="p-3 font-mono text-[#8996A6]">{flow.protocol} ({flow.service})</td>
                            <td className="p-3 text-[#8996A6]">{flow.packet_count}</td>
                            <td className="p-3 font-bold font-mono">
                              <span className={flow.threat_score > 60 ? 'text-[#EF6262]' : 'text-[#35C98A]'}>
                                {flow.threat_score} / 100
                              </span>
                            </td>
                            <td className="p-3">
                              <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                                flow.severity === 'CRITICAL' ? 'bg-[#EF6262]/20 text-[#EF6262]' :
                                flow.severity === 'HIGH' ? 'bg-[#F97316]/20 text-[#F97316]' :
                                flow.severity === 'MEDIUM' ? 'bg-[#F2B84B]/20 text-[#F2B84B]' : 'bg-[#35C98A]/20 text-[#35C98A]'
                              }`}>
                                {flow.severity}
                              </span>
                            </td>
                            <td className="p-3">
                              <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                                flow.anomaly_detected ? 'bg-[#EF6262]/20 text-[#EF6262]' : 'bg-[#35C98A]/20 text-[#35C98A]'
                              }`}>
                                {flow.anomaly_detected ? 'SUSPICIOUS FLOW' : 'NORMAL'}
                              </span>
                            </td>
                            <td className="p-3 text-right">
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setPcapSelectedFlow(flow);
                                  setActiveIncident(flow as any);
                                }}
                                className="px-2.5 py-1 rounded bg-[#1F2937] hover:bg-[#2A374A] text-[#93C5FD] hover:text-white border border-[#3B4D61] text-[10px] font-semibold transition-colors cursor-pointer"
                              >
                                View Details
                ...
                {/* Selected Flow Detail Popover/Box */}
                {pcapSelectedFlow && (
                  <div className="p-4 rounded-xl bg-[#0F151D] border border-[#5B8DEF]/40 space-y-3">
                    <div className="flex items-center justify-between">
                      <h4 className="text-xs font-bold text-[#E8EDF3] flex items-center gap-2">
                        <Activity size={14} className="text-[#5B8DEF]" />
                        <span>Flow Inspection: {pcapSelectedFlow.source} &rarr; {pcapSelectedFlow.destination}</span>
                      </h4>
                      <button
                        onClick={() => setPcapSelectedFlow(null)}
                        className="text-[#8996A6] hover:text-white"
                      >
                        <X size={14} />
                      </button>
                    </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                      <div className="p-2 rounded bg-[#121A24] border border-[#25313E]">
                        <span className="text-[#8996A6] text-[10px] block">TCP Flag State</span>
                        <span className="font-mono text-[#E8EDF3] font-bold">{pcapSelectedFlow.flag}</span>
                      </div>
                      <div className="p-2 rounded bg-[#121A24] border border-[#25313E]">
                        <span className="text-[#8996A6] text-[10px] block">Service Type</span>
                        <span className="font-mono text-[#E8EDF3] font-bold">{pcapSelectedFlow.service}</span>
                      </div>
                      <div className="p-2 rounded bg-[#121A24] border border-[#25313E]">
                        <span className="text-[#8996A6] text-[10px] block">Packet Volume</span>
                        <span className="font-mono text-[#E8EDF3] font-bold">{pcapSelectedFlow.packet_count}</span>
                      </div>
                      <div className="p-2 rounded bg-[#121A24] border border-[#25313E]">
                        <span className="text-[#8996A6] text-[10px] block">Threat Score</span>
                        <span className="font-mono text-[#EF6262] font-bold">{pcapSelectedFlow.threat_score}/100</span>
                      </div>
                    </div>

                    <div className="p-2.5 rounded bg-[#121A24]/70 border border-[#25313E] text-[11px] text-[#8996A6]">
                      <strong>Forensic Assessment:</strong> {pcapSelectedFlow.anomaly_detected
                        ? 'Suspicious flow detected. Host shows abnormal connection patterns matching known reconnaissance/flood signatures.'
                        : 'Normal transport behavior. Connection handshakes and data exchange adhere to benign traffic baselines.'}
                    </div>

                    <div className="flex justify-end pt-1">
                      <button
                        onClick={() => setActiveIncident(pcapSelectedFlow as any)}
                        className="px-3.5 py-1.5 rounded-lg bg-[#EF6262] hover:bg-[#D95353] text-white text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer shadow-md"
                      >
                        <Eye size={14} />
                        <span>View Forensic Details Modal</span>
                      </button>
                    </div>
                  </div>
                )}
        ...
        {/* ========================================================================= */}
        {/* TAB 3: WHAT-IF TESTING (SAFE THREAT SIMULATION LAB) */}
        {/* ========================================================================= */}
        {activeTab === 'simulator' && (
          <div className="p-4 sm:p-6 space-y-6">
            <div className="p-4 rounded-xl bg-[#080C12] border border-[#25313E] space-y-4">
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#25313E] pb-3">
                <div>
                  <h3 className="text-sm font-bold text-[#E8EDF3] flex items-center gap-2">
                    <span>🎛️ Safe Network Threat Simulation Lab</span>
                    <span className="text-[10px] px-2.5 py-0.5 rounded-full font-bold bg-[#35C98A]/15 text-[#35C98A] border border-[#35C98A]/30">
                      SAFE IN-MEMORY SIMULATION
                    </span>
                  </h3>
                  <p className="text-xs text-[#8996A6] mt-0.5">
                    Generate authentic Scapy packet structures in-memory to test the Engine 5 model without transmitting onto physical network cables.
                  </p>
                </div>
                <span className="text-[11px] font-bold text-[#35C98A] bg-[#0E1B15] px-3 py-1 rounded-md border border-[#35C98A]/30">
                  SIMULATION ONLY — NO REAL NETWORK TRAFFIC SENT
                </span>
              </div>

              {/* Scenario selector */}
              <div>
                <label className="text-[11px] font-bold text-[#8996A6] block uppercase tracking-wider mb-2">
                  Select Threat Scenario to Synthesize
                </label>
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                  <button
                    onClick={() => setSimScenario('syn_flood')}
                    className={`p-3 rounded-lg border text-left transition-all cursor-pointer ${
                      simScenario === 'syn_flood'
                        ? 'bg-[#EF6262]/10 border-[#EF6262] text-[#EF6262]'
                        : 'bg-[#151D27] border-[#25313E] text-[#8996A6] hover:text-[#E8EDF3]'
                    }`}
                  >
                    <span className="font-bold text-xs block">🔴 SYN Flood Pattern</span>
                    <span className="text-[10px] opacity-80 mt-1 block">Rapid SYN packets without ACK responses</span>
                  </button>

                  <button
                    onClick={() => setSimScenario('port_scan')}
                    className={`p-3 rounded-lg border text-left transition-all cursor-pointer ${
                      simScenario === 'port_scan'
                        ? 'bg-[#F2B84B]/10 border-[#F2B84B] text-[#F2B84B]'
                        : 'bg-[#151D27] border-[#25313E] text-[#8996A6] hover:text-[#E8EDF3]'
                    }`}
                  >
                    <span className="font-bold text-xs block">🟠 Port Scan Pattern</span>
                    <span className="text-[10px] opacity-80 mt-1 block">Probing sequential TCP destination ports</span>
                  </button>

                  <button
                    onClick={() => setSimScenario('icmp_sweep')}
                    className={`p-3 rounded-lg border text-left transition-all cursor-pointer ${
                      simScenario === 'icmp_sweep'
                        ? 'bg-[#5B8DEF]/10 border-[#5B8DEF] text-[#5B8DEF]'
                        : 'bg-[#151D27] border-[#25313E] text-[#8996A6] hover:text-[#E8EDF3]'
                    }`}
                  >
                    <span className="font-bold text-xs block">🟡 Network Discovery Pattern</span>
                    <span className="text-[10px] opacity-80 mt-1 block">ICMP echo sweeps across network subnet</span>
                  </button>

                  <button
                    onClick={() => setSimScenario('benign_http')}
                    className={`p-3 rounded-lg border text-left transition-all cursor-pointer ${
                      simScenario === 'benign_http'
                        ? 'bg-[#35C98A]/10 border-[#35C98A] text-[#35C98A]'
                        : 'bg-[#151D27] border-[#25313E] text-[#8996A6] hover:text-[#E8EDF3]'
                    }`}
                  >
                    <span className="font-bold text-xs block">🟢 Normal Web Traffic</span>
                    <span className="text-[10px] opacity-80 mt-1 block">Standard HTTP GET & 200 OK data exchange</span>
                  </button>
                </div>
              </div>

              {/* Action Buttons: Export to PCAP & Analyze */}
              <div className="p-3.5 bg-[#121A24] rounded-lg border border-[#25313E] flex flex-wrap items-center justify-between gap-3">
                <div className="text-xs text-[#8996A6]">
                  <span className="font-semibold text-[#E8EDF3] block">Wireshark-compatible PCAP Export:</span>
                  <span>Ye file Wireshark mein open karke packet-level details dekhi ja sakti hain.</span>
                </div>

                <div className="flex items-center gap-2">
                  <button
                    onClick={() => handleExportSimulationPcap(simScenario)}
                    disabled={loading}
                    className="px-4 py-2 rounded-md bg-[#5B8DEF] hover:bg-[#4a7de0] text-white text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
                  >
                    <Download size={14} />
                    <span>💾 EXPORT SIMULATION AS PCAP</span>
                  </button>

                  {exportedPcap && (
                    <button
                      onClick={handleAnalyzeExportedPcapWithEngine5}
                      className="px-4 py-2 rounded-md bg-[#35C98A] hover:bg-[#2EB37A] text-[#0A0E15] text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer"
                    >
                      <Zap size={14} />
                      <span>⚡ ANALYZE WITH ENGINE 5</span>
                    </button>
                  )}
                </div>
              </div>
              ...
              {exportedPcap && (
                <div className="p-3 rounded-lg bg-[#35C98A]/10 border border-[#35C98A]/30 text-xs text-[#35C98A] flex flex-wrap items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <CheckCircle2 size={16} />
                    <span>
                      Simulation PCAP created successfully: <strong>{exportedPcap.filename}</strong> ({exportedPcap.packet_count} packets, {exportedPcap.file_size_bytes} bytes)
                    </span>
                  </div>
                  <a
                    href={exportedPcap.download_url}
                    download={exportedPcap.filename}
                    className="px-3 py-1 rounded bg-[#35C98A] text-[#0A0E15] font-bold text-[11px] hover:bg-[#2EB37A] transition-colors"
                  >
                    Download PCAP File
                  </a>
                </div>
            ...
            {/* Quick Parameter Telemetry Tuner (Preserved from existing functionality) */}
            <div className="rounded-xl bg-[#080C12] border border-[#25313E] p-4 space-y-4">
              <h4 className="text-xs font-bold text-[#E8EDF3] uppercase tracking-wider">
                Direct Telemetry Parameter Tuner (What-If Feature Response)
              </h4>

              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
                <div>
                  <label className="text-[11px] text-[#8996A6] block mb-1">Protocol Type</label>
                  <select
                    value={protocolType}
                    onChange={(e) => setProtocolType(e.target.value)}
                    className="w-full px-2.5 py-1.5 rounded bg-[#151D27] border border-[#25313E] text-[#E8EDF3] text-xs"
                  >
                ...
                <div>
                  <label className="text-[11px] text-[#8996A6] block mb-1">Service Type</label>
                  <select
                    value={service}
                    onChange={(e) => setService(e.target.value)}
                    className="w-full px-2.5 py-1.5 rounded bg-[#151D27] border border-[#25313E] text-[#E8EDF3] text-xs"
                  >
                    <option value="http">Web Service (HTTP)</option>
                    <option value="private">Internal Service (Private)</option>
                    <option value="smtp">Email Service (SMTP)</option>
                    <option value="eco_i">Ping Sweep (ECO_I)</option>
                  </select>
                </div>

                <div>
                  <label className="text-[11px] text-[#8996A6] block mb-1">Connection State (Flag)</label>
                  <select
                    value={flag}
                  ...
                  >
                    ...
                    <option value="S0">S0 (Incomplete / SYN Error)</option>
                    <option value="REJ">REJ (Rejected Connection)</option>
                  </select>
                </div>

                <div>
                  <label className="text-[11px] text-[#8996A6] block mb-1">Connection Attempts (Count)</label>
                  <input
                    type="number"
                    value={count}
                ...
                <div className="sm:col-span-4 flex justify-end">
                  <button
                    ...
                    disabled={loading}
                    className="py-2 px-4 rounded bg-[#5B8DEF] hover:bg-[#4a7de0] text-white font-semibold text-xs transition-colors flex items-center gap-2 cursor-pointer"
                  >
                    {loading ? <RefreshCw size={14} className="animate-spin" /> : <Play size={14} />}
                    <span>Test Parameter Vector</span>
                  </button>
                </div>
              </div>
              ...
              {manualResult && (
                <div className={`p-4 rounded-xl border flex items-center justify-between ${
                  manualResult.anomaly_detected ? 'bg-[#EF6262]/10 border-[#EF6262]/40 text-[#EF6262]' : 'bg-[#35C98A]/10 border-[#35C98A]/40 text-[#35C98A]'
                }`}>
                  <div>
                    <h4 className="font-bold text-sm">
                      {manualResult.anomaly_detected ? 'SUSPICIOUS FLOW DETECTED' : 'NORMAL ACTIVITY DETECTED'}
                    </h4>
                    <p className="text-xs opacity-90 mt-0.5">
                      {manualResult.anomaly_detected
                        ? 'The AI detected abnormal network parameters matching intrusion patterns.'
                        : 'The AI classified the parameter vector as benign connection traffic.'}
                    </p>
                  </div>
                  <div className="text-right">
                    <span className="text-[10px] uppercase text-[#8996A6] block font-medium">Threat Score</span>
                    <span className="font-mono text-xl font-bold">
                      {manualResult.threat_score} / 100
                    </span>
                  </div>
                </div>
              )}

              ...
              <div className="space-y-6">
                ...
                <div className="rounded-lg border border-[#25313E] bg-[#0F151D] p-5 space-y-4">
                  <div className="flex items-center justify-between border-b border-[#25313E]/60 pb-3">
                    <div>
                      ...
                      <p className="text-[11px] text-[#8996A6] mt-0.5">
                        Categorical score breakdown derived from ANVESH Explainable Risk Scoring Engine
                      </p>
                    </div>
                    <div className="text-right font-mono">
                      <span className="text-base font-bold text-[#E8EDF3]">
                        {caseData.risk_score}
                      </span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 text-xs font-mono">
                    {/* Text Signal */}
                    <div className="p-3 rounded bg-[#080C12] border border-[#25313E] space-y-2">
                      <div className="flex justify-between items-center text-[11px]">
                        <span className="text-[#8996A6]">Text / Language Signal</span>
                        <span className="text-[#5B8DEF] font-bold">
                          +{caseData.category_scores?.ml_risk?.score ?? (caseData.ml_signal?.ml_score ?? 0)} / 30
                        </span>
                      </div>
                      <div className="w-full bg-[#151D27] h-1.5 rounded-full overflow-hidden">
                        <div 
                          className="bg-[#5B8DEF] h-full rounded-full transition-all"
                          style={{ width: `${Math.min(100, ((caseData.category_scores?.ml_risk?.score ?? (caseData.ml_signal?.ml_score ?? 0)) / 30) * 100)}%` }}
                        />
                      </div>
                    </div>
                    ...
                    <div className="p-3 rounded bg-[#080C12] border border-[#25313E] space-y-2">
                      <div className="flex justify-between items-center text-[11px]">
                        <span className="text-[#8996A6]">Forensic Authentication</span>
                        <span className="text-[#FF9F0A] font-bold">
                          +{caseData.category_scores?.forensic_auth_risk?.score ?? 0} / 25
                        </span>
                      </div>
                      <div className="w-full bg-[#151D27] h-1.5 rounded-full overflow-hidden">
                        <div 
                          className="bg-[#FF9F0A] h-full rounded-full transition-all"
                          style={{ width: `${Math.min(100, ((caseData.category_scores?.forensic_auth_risk?.score ?? 0) / 25) * 100)}%` }}
                        />
                      </div>
                    </div>
                    ...
                    <div className="p-3 rounded bg-[#080C12] border border-[#25313E] space-y-2">
                      <div className="flex justify-between items-center text-[11px]">
                        <span className="text-[#8996A6]">Infrastructure Intel</span>
                        <span className="text-[#30D158] font-bold">
                          +{caseData.category_scores?.infrastructure_risk?.score ?? 0} / 25
                        </span>
                      </div>
                      <div className="w-full bg-[#151D27] h-1.5 rounded-full overflow-hidden">
                        <div 
                          className="bg-[#30D158] h-full rounded-full transition-all"
                          style={{ width: `${Math.min(100, ((caseData.category_scores?.infrastructure_risk?.score ?? 0) / 25) * 100)}%` }}
                        />
                      </div>
                    </div>
                    ...
                    <div className="p-3 rounded bg-[#080C12] border border-[#25313E] space-y-2">
                      <div className="flex justify-between items-center text-[11px]">
                        <span className="text-[#8996A6]">BEC / Behavioral Rules</span>
                        <span className="text-[#FF453A] font-bold">
                          +{caseData.category_scores?.behavior_bec_risk?.score ?? 0} / 20
                        </span>
                      </div>
                      <div className="w-full bg-[#151D27] h-1.5 rounded-full overflow-hidden">
                        <div 
                          className="bg-[#FF453A] h-full rounded-full transition-all"
                          style={{ width: `${Math.min(100, ((caseData.category_scores?.behavior_bec_risk?.score ?? 0) / 20) * 100)}%` }}
                        />
                      </div>

                    {/* Lookalike Domain (Model 3B) */}
                    <div className="p-3 rounded bg-[#080C12] border border-[#25313E] space-y-2">
                      <div className="flex justify-between items-center text-[11px]">
                        <span className="text-[#8996A6]">Lookalike Domain (M3B)</span>
                        <span className="text-[#BF5AF2] font-bold">
                          +{caseData.category_scores?.lookalike_impersonation_risk?.score ?? 0} / 15
                        </span>
                      </div>
                      <div className="w-full bg-[#151D27] h-1.5 rounded-full overflow-hidden">
                        <div 
                          className="bg-[#BF5AF2] h-full rounded-full transition-all"
                          style={{ width: `${Math.min(100, ((caseData.category_scores?.lookalike_impersonation_risk?.score ?? 0) / 15) * 100)}%` }}
                        />
                      </div>
                    </div>
                    ...
                    <div className="p-3 rounded bg-[#080C12] border border-[#25313E] space-y-2">
                      <div className="flex justify-between items-center text-[11px]">
                        <span className="text-[#8996A6]">Identity Impersonation (M3A)</span>
                        <span className="text-[#FF9F0A] font-bold">
                          +{caseData.category_scores?.identity_impersonation_risk?.score ?? 0} / 15
                        </span>
                      </div>
                      <div className="w-full bg-[#151D27] h-1.5 rounded-full overflow-hidden">
                        <div 
                          className="bg-[#FF9F0A] h-full rounded-full transition-all"
                          style={{ width: `${Math.min(100, ((caseData.category_scores?.identity_impersonation_risk?.score ?? 0) / 15) * 100)}%` }}
                        />
                      </div>
                    </div>
                    </div>
            ...
            {activeSection === 'infrastructure' && (
              <div className="space-y-6">
                ...
                <div className="rounded-lg border border-[#25313E] bg-[#0F151D] p-5 space-y-4">
                  ...
                  {/* Honest Coordinate Presentation / Zero Fabrication */}
                  {caseData.infrastructure?.latitude && caseData.infrastructure?.longitude ? (
                    <div className="p-4 rounded-md bg-[#080C12] border border-[#25313E] flex items-center justify-between text-xs font-mono">
                      <div className="flex items-center gap-2">
                        <Globe size={16} className="text-[#30D158]" />
                        <span>Coordinates mapped from authoritative ASN datacenter record.</span>
                      </div>
                      <span className="text-[#8996A6] text-[10px]">
                        Confidence: IP-Associated Infrastructure
                      </span>
                    </div>
                  ) : (
                    <div className="p-6 rounded-md bg-[#080C12] border border-[#25313E] text-center space-y-1 text-xs font-mono text-[#8996A6]">
                      <Globe size={24} className="mx-auto text-[#8996A6]/40 mb-2" />
                      <div className="text-[#E8EDF3] font-bold">Zero-Fabrication Policy: Map Coordinates UNAVAILABLE</div>
                      <p className="text-[11px] max-w-md mx-auto">
                        No external GeoIP key configured or IP unmapped in local database. 
                        ANVESH strictly prohibits fabricating mock map markers or plausible coordinates.
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="rounded-lg border border-[#25313E] bg-[#0F151D] p-5 space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs font-mono">
                    {(caseData.evidence_gaps?.additional_evidence_options || [
                      ...
                      {
                        evidence_type: 'ISP Subscriber Records (Section 65B)',
                ...
                {caseData.hops && caseData.hops.length > 0 ? (
                  <div className="space-y-3">
                    {caseData.hops.map((hop: any, idx: number) => (
                      <div
                        key={idx}
                        className="p-3 rounded-md bg-[#080C12] border border-[#25313E] space-y-1.5 text-xs font-mono"
                      >
                        <div className="flex items-center justify-between">
                          <span className="font-bold text-[#5B8DEF]">Relay Hop #{hop.hop || idx + 1}</span>
                          {hop.ips && hop.ips.length > 0 && (
                            <span className="text-[11px] text-[#8996A6]">IPs: {hop.ips.join(', ')}</span>
                          )}
                        </div>
                        <p className="text-[11px] text-[#8996A6] font-mono break-all leading-relaxed">
                          {hop.raw}
                        </p>
                      </div>
                    ))}
